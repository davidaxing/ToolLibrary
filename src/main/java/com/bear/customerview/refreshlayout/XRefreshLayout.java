...
import com.bear.customerview.refreshlayout.util.XRefreshScrollingUtil;
...
public class XRefreshLayout extends LinearLayout{
     ...
     * 下拉刷新和上拉加载更多代理
     */
    private XRefreshLayoutDelegate mDelegate;
    /**
     * 手指按下时，y轴方向的偏移量
     */
    ...
    private boolean mHasCustomerHeader = false;
    ...
    private XStickyNavLayout mStickyNavLayout;
     ...
     * 是否有自定义的头布局
     */
    public void setHasCustomerView(boolean statue) {
        this.mHasCustomerHeader = statue;
    }
    ...
    public void onFinishInflate() {
        ...
        } else if (mContentView instanceof XStickyNavLayout) {
            mStickyNavLayout = (XStickyNavLayout) mContentView;
        ...
        if (mRecyclerView != null) {
            mRecyclerView.addOnScrollListener(new OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    int tempScrollY = recyclerView.getScrollY();
                    if ((newState == RecyclerView.SCROLL_STATE_IDLE || newState == RecyclerView.SCROLL_STATE_SETTLING) && mScroolDown && shouldHandleRecyclerViewLoadingMore(mRecyclerView)) {
                        beginLoadingMore();
                    }
                }
...
//            });
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        ...
        return XRefreshScrollingUtil.isAbsListViewToBottom(absListView);
    }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        ...
        return XRefreshScrollingUtil.isRecyclerViewToBottom(recyclerView);
    }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (XRefreshScrollingUtil.isWebViewToBottom(mWebView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isScrollViewToBottom(mScrollView)) {
            return true;
        }
    ...
    private boolean isContentViewToTop() {
        ...
        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mWebView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mScrollView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isAbsListViewToTop(mAbsListView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isRecyclerViewToTop(mRecyclerView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isStickyNavLayoutToTop(mStickyNavLayout)) {
            return true;
        }
    ...
    private boolean handleActionUpOrCancel(MotionEvent event) {
        ...
        if (shouldHandleLoadingMore() && diffY <= -300) {
    ...
    public void beginRefreshing() {
        if (mCurrentRefreshStatus != RefreshStatus.REFRESHING && mDelegate != null) {
            ...
            handleRefreshStatusChanged();
            mDelegate.onXRefreshLayoutBeginRefreshing(this);
        }
    }


    private Runnable mDelayHiddenRefreshViewTask = new Runnable() {
        @Override
        public void run() {
            mCurrentRefreshStatus = RefreshStatus.IDLE;
            hiddenRefreshHeaderView();
...
//            handleRefreshStatusChanged();
//            mRefreshViewHolder.onEndRefreshing();
            mHandler.postDelayed(mDelayHiddenRefreshViewTask, 200);
        }
    }
    ...
    public void beginLoadingMore() {
        if (!mIsLoadingMore && mLoadMoreFooterView != null && mDelegate != null && mDelegate.onXRefreshLayoutBeginLoadingMore(this)) {
            mIsLoadingMore = true;
    ...
    private void showLoadingMoreView() {
        ...
        XRefreshScrollingUtil.scrollToBottom(mScrollView);
        XRefreshScrollingUtil.scrollToBottom(mRecyclerView);
        XRefreshScrollingUtil.scrollToBottom(mAbsListView);
        if (mStickyNavLayout != null) {
    ...
    public void endLoadingMore() {
        if (mIsLoadingMore) {
            if (mIsShowLoadingMoreView) {
                ...
                mHandler.postDelayed(mDelayHiddenLoadingMoreViewTask, 200);
            } else {
     ...
     * @param delegate
     */
    public void setDelegate(XRefreshLayoutDelegate delegate) {
        mDelegate = delegate;
    }

    public interface XRefreshLayoutDelegate {
        /**
         * 开始刷新
         */
        void onXRefreshLayoutBeginRefreshing(XRefreshLayout refreshLayout);
        /**
         ...
         */
        boolean onXRefreshLayoutBeginLoadingMore(XRefreshLayout refreshLayout);
    }
