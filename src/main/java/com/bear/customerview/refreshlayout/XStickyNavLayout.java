...
public class XStickyNavLayout extends LinearLayout {
    ...
    public XRefreshLayout mRefreshLayout;

    public XStickyNavLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 3) {
            throw new IllegalStateException(XStickyNavLayout.class.getSimpleName() + "必须有且只有三个子控件");
        }
    ...
    public boolean isContentViewToTop() {
        ...
        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectWebView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectScrollView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isAbsListViewToTop(mDirectAbsListView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isRecyclerViewToTop(mDirectRecyclerView)) {
            return true;
        }
    ...
    private boolean isViewPagerContentViewToTop() {
        ...
        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedWebView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedScrollView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isAbsListViewToTop(mNestedAbsListView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isRecyclerViewToTop(mNestedRecyclerView)) {
            return true;
        }
    ...
    private void regetNestedContentView() {
        ...
        if (adapter instanceof FragmentPagerAdapter || adapter instanceof FragmentStatePagerAdapter) {
            ...
            throw new IllegalStateException(XStickyNavLayout.class.getSimpleName() + "的第三个子控件为ViewPager时，其adapter必须是FragmentPagerAdapter或者FragmentStatePagerAdapter");
        }
    }

    public void setRefreshLayout(XRefreshLayout refreshLayout) {
    ...
    public boolean shouldHandleLoadingMore() {
        ...
        if (XRefreshScrollingUtil.isWebViewToBottom(mDirectWebView)) {
            return true;
        }

        if (XRefreshScrollingUtil.isScrollViewToBottom(mDirectScrollView)) {
            return true;
        }
        ...
        if (mDirectViewPager != null) {
            ...
            if (XRefreshScrollingUtil.isWebViewToBottom(mNestedWebView)) {
                return true;
            }

            if (XRefreshScrollingUtil.isScrollViewToBottom(mNestedScrollView)) {
                return true;
            }
    ...
    public void scrollToBottom() {
        XRefreshScrollingUtil.scrollToBottom(mDirectScrollView);
        XRefreshScrollingUtil.scrollToBottom(mDirectRecyclerView);
        XRefreshScrollingUtil.scrollToBottom(mDirectAbsListView);

        if (mDirectViewPager != null) {
            if (mNestedContentView == null) {
                regetNestedContentView();
            }
            XRefreshScrollingUtil.scrollToBottom(mNestedScrollView);
            XRefreshScrollingUtil.scrollToBottom(mNestedRecyclerView);
            XRefreshScrollingUtil.scrollToBottom(mNestedAbsListView);
        }
    }
}