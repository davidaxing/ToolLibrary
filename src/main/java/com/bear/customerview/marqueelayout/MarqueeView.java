package com.bear.customerview.marqueelayout;
...
public class MarqueeView extends ViewFlipper {
    ...
    private InitViewCallBack mCallBack;
    ...
    // 根据公告字符串列表启动轮播
    public void startWithList(List<String> notices, InitViewCallBack _callBack) {
    ...
    public boolean start() {
        ...
        for (int i = 0; i < notices.size(); i++) {
            final View customerview = View.inflate(mContext, R.layout.item_marquee_layout, null);
            customerview.setClickable(true);
            customerview.setFocusable(true);
            customerview.setFocusableInTouchMode(true);
            mCallBack.init(i, customerview);
//            final TextView textView = createTextView(notices.get(i), i);
            final int finalI = i;
            customerview.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (onItemClickListener != null) {
                        onItemClickListener.onItemClick(finalI, customerview);
                    }
                }
            });
    ...
    public interface OnItemClickListener {
        void onItemClick(int position, View root);
    }

    public interface InitViewCallBack {
        void init(int position, View root);
    }
